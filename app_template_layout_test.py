...
location = params.get("location", "Seng Kang, Singapore")
coords = params.get("coords", "1.3871, 103.8915")
radius_m_raw = params.get("radius", "1500")
try:
    radius_m = max(500, min(3000, int(radius_m_raw)))
except (TypeError, ValueError):
    radius_m = 1500
interests_raw = params.get("interests", "cheap food,grocery,event,deal")
...
safe_interests_value = escape(interests_value)
safe_radius = escape(str(radius_m))
...
.area-form{margin-top:10px}.area-label{font-size:11.8px;color:var(--muted)!important;font-weight:750;margin:8px 0 5px 2px}.area-input{width:100%;min-height:42px;border:1px solid #D8DFEA;border-radius:13px;background:white;padding:0 13px;font-size:12.5px;color:#4B5565!important;margin-bottom:8px;box-shadow:0 2px 8px rgba(23,43,77,.025);box-sizing:border-box}.area-input:focus{outline:2px solid #C7DBFF;border-color:#8BB8FF}.area-row{display:grid;grid-template-columns:1fr 1fr;gap:8px}.save-btn{width:100%;min-height:44px;border:0;border-radius:13px;background:linear-gradient(90deg,#0D6EFD,#2563EB);color:white!important;font-weight:900;margin-top:10px;box-shadow:0 8px 18px rgba(13,110,253,.22);cursor:pointer}.save-btn:hover{filter:brightness(.98)}
...
@media(max-height:760px){:root{--app-h:calc(100dvh - .9rem);--chat-body-h:clamp(260px, calc(100dvh - 405px), 420px);--picks-body-h:clamp(370px, calc(100dvh - 165px), 620px)}.pick{min-height:108px}.inputbar{min-height:52px}.quick{min-height:39px}.brand{margin-bottom:12px}.area-input{min-height:36px;margin-bottom:6px}.location-field{min-height:48px}.nav a{padding:8px 10px}.tag{padding:5px 8px}.sidebar-note{display:none}.sidebar-card,.chat-card,.picks-card,.full-card{padding-top:22px}}
</style>
...
<div class="nav"><a class="{active('today')}" href="{page_url('today')}" target="_self">● GoAround Today</a><a class="{active('business')}" href="{page_url('business')}" target="_self">○ Business Promotion</a><a class="{active('about')}" href="{page_url('about')}" target="_self">○ What is GoAround?</a></div>
<div class="side-title">My area</div><div class="subtitle">Manual area state. Browser location detection comes later.</div>
<form class="area-form" method="get" target="_self">
  <input type="hidden" name="page" value="{escape(page)}" />
  <div class="area-label">Try location</div>
  <div class="field">Manual / Custom location</div>
  <div class="area-label">Block / place / postal code</div>
  <input class="area-input" name="location" value="{safe_location}" />
  <div class="area-row"><div><div class="area-label">Coordinates</div><input class="area-input" name="coords" value="{safe_coords}" /></div><div><div class="area-label">Radius metres</div><input class="area-input" name="radius" value="{safe_radius}" inputmode="numeric" /></div></div>
  <div class="area-label">Interests, comma separated</div>
  <input class="area-input" name="interests" value="{safe_interests_value}" />
  <div class="tag-wrap">{render_tags(interests)}</div>
  <button class="save-btn" type="submit">💾 Save my area</button>
</form>
...
<div style="margin-top:12px"><span class="status">☀️ 35.0°C Sunny</span><span class="status">📍 {safe_location}</span><span class="status">◎ Within {safe_radius_label}</span></div>
<div class="chatbox">
  ...
  <div>🤖 <span class="bubble">Here are some budget-friendly options near {safe_location} within {safe_radius_label}.</span></div>
</div>
...
<div class="inputbar"><div style="text-align:center">📎</div><div class="muted">Ask GoAround about {safe_location}...</div><div class="send">➤</div></div>
...
<div class="main-shell-title"><div><h2>Today’s Picks</h2><div class="muted">Curated for {safe_location} based on your area and interests.</div></div><div class="view-all">View all</div></div>
<div class="picklist">
  <div class="pick"><b>🤖 {safe_first_interest.title()} near {safe_location}</b><br><span class="muted">Local source · within {safe_radius_label}</span><br>Placeholder pick will later come from source-backed data.<br><span class="visit">Visit Website</span></div>
  <div class="pick"><b>🎉 What’s happening around {safe_location}</b><br><span class="muted">Events · within {safe_radius_label}</span><br>Local activities and useful updates near your selected area.<br><span class="visit">Visit Website</span></div>
  <div class="pick"><b>🏷️ Grocery and deal updates</b><br><span class="muted">Interests · {escape(', '.join(interests[:3]))}</span><br>Weekly grocery offers and deal sources near your area.<br><span class="visit">Visit Website</span></div>
</div>
...
<div class="muted">Create a local promotion that can appear in Today’s Picks near {safe_location}.</div>
...
<div class="preview-card"><div class="tag">FOOD & DINING</div><h2 style="margin-top:16px!important">50% Off Chicken Rice</h2><div class="muted">Fresh chicken, fragrant rice and homemade chilli near {safe_location}.</div><br><span class="status">📍 {safe_location}</span><br><span class="visit">View details ↗</span></div>
</div>
''', unsafe_allow_html=True)
    else:
        st.markdown(f'''
...
<div class="muted">A source-backed local discovery assistant for Singapore. Current selected area: {safe_location} within {safe_radius_label}.</div>
