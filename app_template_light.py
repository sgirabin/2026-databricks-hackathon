...
html,body,.stApp,[data-testid="stAppViewContainer"],[data-testid="stSidebar"],[data-testid="block-container"]{background:var(--bg)!important;color:var(--text)!important;color-scheme:light!important;}
[data-testid="stHeader"]{background:rgba(247,250,252,.97)!important;}
.main .block-container{padding:12px 18px 6px 18px!important;max-width:1580px!important;}
section[data-testid="stSidebar"]{background:#F4F7FB!important;border-right:1px solid #E5EAF3!important;}
section[data-testid="stSidebar"] .block-container{padding:14px 12px 8px 12px!important;}
*{box-sizing:border-box;}
/* hard override for browser/system dark mode and dark-reader style leakage */
p,span,label,div,h1,h2,h3,h4,h5,h6,li{color:inherit;}
.stMarkdown,.stCaption,.stText,.stRadio,.stSelectbox,.stTextInput,.stMultiSelect,.stSlider{color:var(--text)!important;}
input,textarea,select,[data-baseweb="select"]>div,[data-baseweb="input"]>div,[data-baseweb="textarea"]>div{background:#FFFFFF!important;color:#172B4D!important;border-color:#D8DFEA!important;box-shadow:none!important;}
input::placeholder,textarea::placeholder{color:#98A2B3!important;opacity:1!important;}
[data-baseweb="select"] span,[data-baseweb="select"] div,[data-baseweb="popover"] div,[data-baseweb="menu"] div{color:#172B4D!important;background:#FFFFFF!important;}
...
button{color:#172B4D!important;}
button[kind="primary"]{background:#0D6EFD!important;color:white!important;border-radius:12px!important;border:0!important;}
...
.brand{display:flex;gap:10px;align-items:center;margin:0 0 12px 0;}
.logo-pin{width:40px;height:40px;border-radius:50% 50% 50% 8px;background:linear-gradient(145deg,#0D6EFD,#20B2AA);transform:rotate(-45deg);position:relative;box-shadow:0 10px 22px rgba(13,110,253,.20);flex:0 0 auto;}
.logo-pin:after{content:"";width:16px;height:16px;background:white;border-radius:50%;position:absolute;left:12px;top:12px;}
.brand h1{font-size:22px;line-height:1;margin:0;color:#0D2B5C;font-weight:850;letter-spacing:-.02em}.brand h1 span{color:#10B981}.brand p{font-size:12px;margin:4px 0 0 0;color:#596579;}
.side-section{background:#FFFFFF;border:1px solid #E6EAF2;border-radius:18px;padding:14px;margin-top:12px;box-shadow:0 6px 20px rgba(23,43,77,.035);}
.main-card{height:calc(100vh - 92px);min-height:690px;background:#FFFFFF;border:1px solid #E6EAF2;border-radius:22px;box-shadow:0 12px 34px rgba(23,43,77,.06);padding:18px;overflow:hidden;display:flex;flex-direction:column;}
.business-card{height:calc(100vh - 128px);min-height:650px;background:#FFFFFF;border:1px solid #E6EAF2;border-radius:22px;box-shadow:0 12px 34px rgba(23,43,77,.06);padding:18px;overflow:auto;}
.card-title{font-size:23px;font-weight:850;color:#172B4D;margin:0;letter-spacing:-.015em}.muted{color:#667085;font-size:13px}.small{font-size:12px;color:#667085}.footer-note{text-align:center;color:#667085;font-size:12px;margin-top:4px;}
.chat-scroll{flex:1;min-height:260px;overflow:auto;border-radius:18px;background:linear-gradient(#fff,#fbfdff);padding:8px 6px;margin:8px 0 10px 0;}
.chat-row{display:flex;margin:10px 0}.chat-row.assistant{justify-content:flex-start}.chat-row.user{justify-content:flex-end}.chat-bubble{max-width:78%;border-radius:18px;padding:12px 14px;font-size:14px;line-height:1.45}.assistant .chat-bubble{background:#F1F5F9;color:#0F172A;border-bottom-left-radius:6px}.user .chat-bubble{background:#0D6EFD;color:white;border-bottom-right-radius:6px}.avatar{width:36px;height:36px;border-radius:50%;display:flex;align-items:center;justify-content:center;margin-right:10px;background:#EAF2FF;font-size:19px;flex:0 0 auto;}
.bot-hero{text-align:center;margin:6px 0 10px}.bot-face{font-size:72px;line-height:1;margin:4px 0}.bot-hero h2{font-size:21px;margin:0;color:#172B4D}.prompt-row{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin:4px 0 10px 0;}
.pick-list{flex:1;overflow:auto;padding-right:4px;margin-top:8px;}.pick{border:1px solid #E6EAF2;border-radius:18px;padding:14px;margin:0 0 12px 0;background:#FFFFFF;box-shadow:0 5px 18px rgba(23,43,77,.04)}.pick-head{display:flex;align-items:center;gap:8px;margin-bottom:8px}.rank{background:#0D6EFD;color:#fff;border-radius:8px;padding:3px 8px;font-weight:850;font-size:12px}.badge{display:inline-block;border-radius:999px;padding:4px 8px;font-size:12px;font-weight:750;background:#EEF4FF;color:#175CD3}.badge.green{background:#DFF8EE;color:#047857}.badge.purple{background:#F0E8FF;color:#6941C6}.badge.orange{background:#FFF2E0;color:#C05621}.pick-title{font-size:17px;font-weight:850;color:#172B4D;margin:0 0 4px 0}.pick-desc{font-size:13.5px;color:#344054;margin:0 0 10px 0}.pick-footer{display:flex;justify-content:space-between;align-items:center;color:#667085;font-size:12px}.visit{border:1px solid #E6EAF2;border-radius:12px;padding:7px 11px;color:#0D6EFD!important;font-weight:800;background:white;text-decoration:none}.big-icon{float:right;font-size:44px;margin-left:8px}.kpi-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin:0 0 12px 0}.kpi{background:#FFFFFF;border:1px solid #E6EAF2;border-radius:18px;padding:14px;box-shadow:0 5px 18px rgba(23,43,77,.04)}.kpi b{font-size:25px;color:#172B4D}.phone{border:8px solid #EEF2F7;border-radius:36px;padding:14px;background:white;max-width:330px;margin:auto}.food-img{height:150px;border-radius:16px;background:linear-gradient(135deg,#FFE6C7,#F8B26A);display:flex;align-items:center;justify-content:center;font-size:72px;margin:8px 0}.primary{background:#0D6EFD;color:white!important;border-radius:12px;padding:12px 14px;text-align:center;font-weight:800;margin-top:8px}.intro-card{background:#FFFFFF;border:1px solid #E6EAF2;border-radius:22px;box-shadow:0 12px 34px rgba(23,43,77,.06);padding:22px;min-height:calc(100vh - 92px);}
</style>
...
with st.sidebar:
    st.markdown("<div class='brand'><div class='logo-pin'></div><div><h1 style='font-size:22px;line-height:1;margin:0;color:#0D2B5C;font-weight:850'>GoAround <span style='color:#10B981'>SG</span></h1><p style='font-size:12px;margin:4px 0 0;color:#596579'>AI local discovery assistant for Singapore</p></div></div>", unsafe_allow_html=True)
...
if page == "GoAround Today":
    ...
    with chat_col:
        ...
        st.markdown("<div class='footer-note'>GoAround can make mistakes. Please check details at the source.</div></div>", unsafe_allow_html=True)
    ...
    with picks_col:
        ...
        st.markdown("<div class='pick-list'>", unsafe_allow_html=True)
    ...
    st.markdown("<div class='kpi-grid'><div class='kpi'>🏷️<br><span class='muted'>Active Promotions</span><br><b>3</b><br><span class='badge green'>▲ 1</span></div><div class='kpi'>🖱️<br><span class='muted'>Clicks (7 days)</span><br><b>128</b><br><span class='badge green'>▲ 18%</span></div><div class='kpi'>🔖<br><span class='muted'>Saves (7 days)</span><br><b>47</b><br><span class='badge green'>▲ 21%</span></div><div class='kpi'>👁️<br><span class='muted'>Views (7 days)</span><br><b>612</b><br><span class='badge green'>▲ 12%</span></div></div>", unsafe_allow_html=True)
