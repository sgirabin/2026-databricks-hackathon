...
st.markdown("""
<style>
.main .block-container {padding-top:2.0rem; max-width:1500px; padding-bottom:.35rem;}
...
.status-line {border:1px solid #e5e7eb;border-radius:14px;padding:6px 10px;background:white;margin:2px 0 8px 0;box-shadow:0 3px 10px rgba(15,23,42,.04);}
.status-title {font-size:13px;font-weight:800;color:#0f172a;margin-right:8px;}
.badge {display:inline-block;border-radius:999px;padding:3px 7px;font-size:11px;font-weight:700;margin:0 4px 3px 0;border:1px solid #dbeafe;background:#eff6ff;color:#1d4ed8;vertical-align:middle;}
...
button[kind="secondary"] {min-height:2.25rem;}
</style>
    ...
    try:
        ...
        for feature in gj.get("features", []):
            ...
            if geom.get("type") == "Point" and len(coords) >= 2:
                ...
                rows.append({"category": category, "name": str(name), "lat": float(coords[1]), "lon": float(coords[0])})
        return pd.DataFrame(rows)
    except Exception:
        return pd.DataFrame(columns=["category", "name", "lat", "lon"])


@st.cache_data(ttl=86400, show_spinner=True)
def load_public_cards(lat: float, lon: float, radius_m: int) -> list[PickCard]:
    frames = [load_geojson_points(ds, cat) for cat, ds in DATASETS.items()]
...
def render_chat_message(msg: dict[str, str]) -> None:
    ...
    content = md_to_html(msg.get("content", ""))
    avatar = "<div class='chat-avatar'>🤖</div>" if role == "assistant" else ""
    st.markdown(f"<div class='chat-row {role}'>{avatar}<div class='chat-bubble'>{content}</div></div>", unsafe_allow_html=True)
...
main_tab, business_tab, about_tab = st.tabs(["GoAround Today", "Business Promotion", "About Databricks"])

with main_tab:
    st.markdown(f"""
<div class='status-line'>
  <span class='status-title'>Today</span>
  <span class='badge {'badge-good' if lakehouse_mode else 'badge-warn'}'>Data: {'Lakehouse' if lakehouse_mode else 'Public API fallback'}</span>
  <span class='badge {'badge-good' if genai_mode else 'badge-warn'}'>AI: {'Model Serving' if genai_mode else 'Safe fallback'}</span>
  <span class='badge'>Area: {esc(profile['address'][:80])}</span>
