...
.main .block-container{padding:1.9rem 1rem .25rem 1rem!important;max-width:1580px!important;}
...
section[data-testid="stSidebar"] .block-container{padding:.55rem .55rem .25rem .55rem!important;}
section[data-testid="stSidebar"] div[data-testid="stVerticalBlock"]{gap:.18rem!important;}
div[data-testid="stVerticalBlock"]{gap:.34rem!important;}
/* force light mode even under browser/system dark mode */
...
[data-baseweb="tag"]{background:#EEF4FF!important;color:#175CD3!important;min-height:1.35rem!important;font-size:.78rem!important;}
...
section[data-testid="stSidebar"] button{min-height:2rem!important;padding:.18rem .6rem!important;}
section[data-testid="stSidebar"] label{font-size:.8rem!important;margin-bottom:.05rem!important;}
section[data-testid="stSidebar"] [data-baseweb="select"]>div{min-height:2.05rem!important;}
section[data-testid="stSidebar"] input{min-height:1.95rem!important;}
...
.brand{display:flex;gap:.55rem;align-items:center;margin-bottom:.35rem;}.pin{width:34px;height:34px;border-radius:50% 50% 50% 8px;background:linear-gradient(145deg,#0D6EFD,#20B2AA);transform:rotate(-45deg);position:relative;box-shadow:0 8px 18px rgba(13,110,253,.18);flex:0 0 auto}.pin:after{content:"";width:13px;height:13px;background:#fff;border-radius:50%;position:absolute;left:10.5px;top:10.5px}.brand h1{font-size:18px;margin:0;line-height:1;color:#0D2B5C!important;font-weight:850}.brand h1 b{color:#10B981!important}.brand p{font-size:10.5px;margin:3px 0 0 0;color:#596579!important}.weather-card{background:#FFFFFF;border:1px solid #E6EAF2;border-radius:16px;padding:.5rem .6rem;margin:.15rem 0 .3rem;box-shadow:0 5px 16px rgba(23,43,77,.035)}.weather-card .temp{font-size:1.2rem;font-weight:850;color:#172B4D!important}.weather-card .sub{font-size:.75rem;color:#667085!important}.chips{display:flex;gap:.35rem;flex-wrap:wrap;margin:.05rem 0 .5rem}.chip{border-radius:999px;padding:.26rem .55rem;font-size:.76rem;font-weight:750;background:#EEF4FF;color:#175CD3!important;border:1px solid #D8E7FF}.chip.green{background:#DFF8EE;color:#047857!important;border-color:#BBF7D0}.chip.warn{background:#FFF7E6;color:#B45309!important;border-color:#FDE68A}.card-title{font-size:1.35rem;font-weight:850;margin:0;color:#172B4D!important}.section-title{font-size:1rem;font-weight:850;margin:0;color:#172B4D!important}.chat-row{display:flex;margin:.6rem 0}.chat-row.assistant{justify-content:flex-start}.chat-row.user{justify-content:flex-end}.chat-bubble{max-width:80%;border-radius:18px;padding:.7rem .85rem;font-size:.9rem;line-height:1.45}.assistant .chat-bubble{background:#F1F5F9!important;color:#0F172A!important;border-bottom-left-radius:6px}.user .chat-bubble{background:#0D6EFD!important;color:#fff!important;border-bottom-right-radius:6px}.avatar{width:34px;height:34px;border-radius:50%;display:flex;align-items:center;justify-content:center;margin-right:.55rem;background:#EAF2FF;font-size:1rem;flex:0 0 auto}.bot-hero{text-align:center;margin:.35rem 0 .55rem}.bot-face{font-size:3rem;line-height:1}.pick-card{border:1px solid #E6EAF2;border-radius:18px;padding:.8rem;margin-bottom:.6rem;background:#FFFFFF!important;box-shadow:0 5px 16px rgba(23,43,77,.035)}.pick-title{font-size:.96rem;font-weight:850;color:#172B4D!important;margin:0 0 .18rem}.pick-desc{font-size:.82rem;color:#344054!important;margin:0 0 .5rem}.rank{background:#0D6EFD;color:#fff!important;border-radius:8px;padding:.12rem .4rem;font-weight:850;font-size:.72rem}.badge{display:inline-block;border-radius:999px;padding:.2rem .46rem;font-size:.7rem;font-weight:750;background:#EEF4FF;color:#175CD3!important}.badge.green{background:#DFF8EE;color:#047857!important}.badge.purple{background:#F0E8FF;color:#6941C6!important}.badge.orange{background:#FFF2E0;color:#C05621!important}.visit{border:1px solid #E6EAF2;border-radius:12px;padding:.36rem .62rem;color:#0D6EFD!important;font-weight:800;background:white;text-decoration:none}.kpi{background:#fff;border:1px solid #E6EAF2;border-radius:18px;padding:.7rem;box-shadow:0 5px 16px rgba(23,43,77,.035)}.kpi b{font-size:1.35rem;color:#172B4D!important}.phone{border:7px solid #EEF2F7;border-radius:32px;padding:.8rem;background:white;max-width:315px;margin:auto}.food-img{height:125px;border-radius:16px;background:linear-gradient(135deg,#FFE6C7,#F8B26A);display:flex;align-items:center;justify-content:center;font-size:3.4rem;margin:.45rem 0}.primary{background:#0D6EFD;color:white!important;border-radius:12px;padding:.65rem .8rem;text-align:center;font-weight:800;margin-top:.45rem}.footer-note{text-align:center;font-size:.72rem;color:#667085!important;margin-top:.2rem}.compact-footer{font-size:.74rem;color:#667085!important;line-height:1.2;margin-top:.25rem;}
</style>
...
<div class='pick-card'>
  ...
  <div style='float:right;font-size:2.1rem;margin-left:8px'>{icon}</div>
  <div class='pick-title'>{esc(c.title)}</div>
  <div class='pick-desc'>{esc(c.description)}</div>
  <div style='display:flex;justify-content:space-between;align-items:center;font-size:.74rem;color:#667085'><span>📍 Within your area</span><a class='visit' href='{esc(c.source_url)}' target='_blank'>View details ↗</a></div>
</div>
""", unsafe_allow_html=True)
...
weather_slot.markdown(
    f"<div class='weather-card'><div style='display:flex;justify-content:space-between;align-items:center'><div><div class='temp'>{esc(weather.get('forecast') or 'Weather')}</div><div class='sub'>{esc(weather.get('area') or profile['address'][:28])}</div></div><div style='font-size:1.8rem'>{weather_icon(weather.get('forecast'))}</div></div></div>",
    unsafe_allow_html=True,
)
...
if page == "GoAround Today":
    chat_col, picks_col = st.columns([1.72, 1.08], gap="small")
    with chat_col:
        with st.container(border=True):
